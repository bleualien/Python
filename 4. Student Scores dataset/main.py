...
def predict_subject(subject, hours):
    ...
    # Train model
    model = LinearRegression() #Linear Regression model (straight-line formula y = mx + c)
    model.fit(X_train, y_train) #finding the best slope (m) and intercept (c) to predict marks.
    
    # Predict marks (pass DataFrame with column name)
    pred = model.predict(pd.DataFrame({'weekly_self_study_hours':[hours]}))[0] #question sodhe paxi hour ma std i/p aauxa [0]prediction(list/array)
    
    # Evaluate MSE
    mse = mean_squared_error(y_test, model.predict(X_test))#samller MSE = better model
    
    return round(pred, 2), round(mse, 2) #predicted score, error

# Step 6: Predict all subjects and store results
predicted_scores = { #stds info + all predicted marks.
...
mse_dict = {}#MSE for each subject.

for subject in subjects:#predict_subject() → gets predicted marks and MSE
    pred, mse = predict_subject(subject, weekly_hours)
    predicted_scores[subject.replace('_score','').title()] = pred #attributes name haru ramro banauxa
